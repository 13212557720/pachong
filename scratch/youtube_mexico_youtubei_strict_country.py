...
DEFAULT_OUT_DIR = Path("output_youtube_mexico_20k_100k_strict_mx")
...
def is_mexico_country(value: str) -> bool:
    ...
    return normalized in {"墨西哥", "méxico", "mexico", "mx"} or "mexico" in normalized or "méxico" in normalized
...
def write_outputs(rows: list[Lead], out_dir: Path) -> None:
    ...
    df.to_csv(out_dir / "youtube_mexico_20k_100k_strict_mx.csv", index=False, encoding="utf-8-sig")
    df.to_excel(out_dir / "youtube_mexico_20k_100k_strict_mx.xlsx", index=False)
    clean_output_files(out_dir)


def load_existing(out_dir: Path) -> list[Lead]:
    path = out_dir / "youtube_mexico_20k_100k_strict_mx.csv"
    if not path.exists():
        return []
    df = pd.read_csv(path)
    df = df.rename(
...
def clean_output_files(out_dir: Path) -> None:
    for name in ["youtube_mexico_20k_100k_strict_mx.csv", "youtube_mexico_20k_100k_strict_mx.xlsx"]:
        path = out_dir / name
        if not path.exists():
            continue
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--min-subscribers", type=int, default=20_000)
    parser.add_argument("--max-subscribers", type=int, default=100_000)
    ...
    for query in queries:
        ...
        for page_index in range(args.max_pages_per_query):
            for renderer in find_channel_renderers(data):
                ...
                if not is_mexico_country(about.get("country", "")):
                    continue
                subscriber_count = about.get("subscriber_count") or item["subscriber_count"]
                if not (args.min_subscribers <= subscriber_count <= args.max_subscribers):
                    continue
